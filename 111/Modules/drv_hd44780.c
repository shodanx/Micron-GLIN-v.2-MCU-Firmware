...
// http://electronix.ru/forum/index.php?showtopic=97581
// http://forum.easyelectronics.ru/viewtopic.php?f=9&t=9727
//please send your request to me by e mail, then i can send you more detial, thanks
//my e mail is Brian@winstar.com.tw
  DelayResolution100us(HD44780_POWER_UP_DLY);
  DelayResolution100us(HD44780_POWER_UP_DLY);
  DelayResolution100us(HD44780_POWER_UP_DLY);
  DelayResolution100us(HD44780_POWER_UP_DLY);
...
//Загрузка спец.символов
  // "/ч"
const HD44780_STRING_DEF _hour[]={0x01,0x02,0x04,0x08,0x15,0x07,0x01,0x00};
  HD44780_WrCGRAM(_hour,7);
  // "градусы"
const HD44780_STRING_DEF _grad[]={0x06,0x09,0x09,0x06,0x00,0x00,0x00,0x00};
  HD44780_WrCGRAM(_grad,6);
  // "/c"
//const HD44780_STRING_DEF _sec1[]={0x01,0x02,0x04,0x08,0x17,0x04,0x07,0x00};
const HD44780_STRING_DEF _sec[]={0x01,0x02,0x04,0x0B,0x14,0x04,0x03,0x00};
  HD44780_WrCGRAM(_sec,5);
  // "м"
const HD44780_STRING_DEF _m[]={0x11,0x1B,0x15,0x11,0x11,0x00,0x00,0x00};
  HD44780_WrCGRAM(_m,4);
  // "к"
const HD44780_STRING_DEF _k[]={0x09,0x0A,0x0C,0x0A,0x09,0x00,0x00,0x00};
  HD44780_WrCGRAM(_k,3);
...
/*******************************************************************
 *Function Name:ANSI_LCD
 *
 *Parameters: 
 LPC_INT8U temp - ASCII символ
 *
 *Description:
 *
 *Returns:
 LPC_INT8U - символ LCD
 *
 *******************************************************************/
const Int8U sym_ASCII_LCD[]={
...
Int8U ANSI_LCD(Int8U temp)
{
if (temp==0xa8) return (sym_ASCII_LCD[64]);//"Ё"
else if (temp==0xb8) return(sym_ASCII_LCD[65]);//"ё"
else if (temp<0xc0) return(temp);
else return(sym_ASCII_LCD[temp-0xc0]);
}
...
HD44780_ERROR_CODE_DEF ErrorRes = HD44780_OK;
  ...
  while (*DataStr)
  {
    ...
    HD44780WrData(ANSI_LCD(*DataStr));//HD44780WrData(*DataStr);
#if HD4780_WR == 0
    if(HD4478_Ctrl.AC_Direction)
    {
